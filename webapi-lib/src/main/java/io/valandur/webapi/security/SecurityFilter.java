...
import static io.valandur.webapi.security.SecurityService.ACCESS_CONTROL_ALLOW_HEADERS;
import static io.valandur.webapi.security.SecurityService.ACCESS_CONTROL_ALLOW_METHODS;
import static io.valandur.webapi.security.SecurityService.ACCESS_CONTROL_ALLOW_ORIGIN;
import static io.valandur.webapi.security.SecurityService.ACCESS_CONTROL_HEADERS;
import static io.valandur.webapi.security.SecurityService.ACCESS_CONTROL_METHODS;
import static io.valandur.webapi.security.SecurityService.ACCESS_CONTROL_ORIGIN;
import static io.valandur.webapi.security.SecurityService.API_KEY;
import static io.valandur.webapi.security.SecurityService.X_FORWARDED_FOR;
...
public class SecurityFilter implements ContainerRequestFilter, ContainerResponseFilter {
  ...
  public SecurityFilter() {
    var webapi = WebAPI.getInstance();

    srv = webapi.getSecurityService();
  ...
  private String getRealAddress(String address, String forwardedFor) {
    ...
    if (!srv.containsProxyIP(address)) {
      logger.warn(address + " sent " + X_FORWARDED_FOR
